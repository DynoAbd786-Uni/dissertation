...
class PipeSimulation2D:
    ...
    def get_spatial_flow_profile(self):
        ...
        profile_type = profile_config.get("type", "poiseuille").lower()
    ...
    def setup_boundary_conditions(self):
        ...
        if self.use_time_dependent_zou_he:
            ...
            if self.spatial_profile is not None:
                print(f"Spatial profile: {self.spatial_profile.__class__.__name__}")
            else:
            ...
            # Standard ZouHeBC for steady-state boundary conditions with spatial profile
            if self.spatial_profile is None:
                bc_inlet = ZouHeBC(
                    bc_type="velocity",
                ...
                bc_inlet = ZouHeBC(
                    bc_type="velocity",
                    profile=spatial_profile_function,
                    indices=inlet,
                )
    ...
    def update_spatial_profile_scale(self, scale_factor):
        ...
        """
        if hasattr(self.spatial_profile, 'update_scale_factor'):
