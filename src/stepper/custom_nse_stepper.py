...
class CustomNSEStepper(Stepper):
    """
    Custom Navier-Stokes stepper that supports time-dependent boundary conditions
    and non-Newtonian fluid models. This class directly inherits from Stepper
    rather than IncompressibleNavierStokesStepper to allow for more customization.
    """
    ...
    def __init__(
        ...
        # Initialize parent class (Stepper)
        super().__init__(grid, boundary_conditions)
        
        # Construct the appropriate collision operator
        if collision_type == "BGK":
        ...
        elif collision_type == "BGKNonNewtonian":
            # Use custom non-Newtonian collision operator
            self.collision = BGKNonNewtonian(
            ...
            print(f"Using Non-Newtonian BGK collision operator with parameters:")
            print(f"  μ₀={mu_0}, μ∞={mu_inf}, λ={lambda_cy}, n={n}, a={a}")
            print(f"  dx={dx_physical}, dt={dt_physical}")
        else:
            raise ValueError(f"Unsupported collision type: {collision_type}")

        # Apply forced collision if needed
    ...
    def prepare_fields(self, initializer=None):
        ...
        _, f_0, f_1, missing_mask, bc_mask = create_nse_fields(
            grid=self.grid, velocity_set=self.velocity_set, compute_backend=self.compute_backend, 
            precision_policy=self.precision_policy
        )
    ...
    def jax_implementation(self, f_0, f_1, bc_mask, missing_mask, timestep):
        """
        JAX implementation with support for time-dependent boundary conditions
        """
        ...
        # Apply collision - Note: BGKNonNewtonian will use the same interface
    ...
    def _construct_warp(self):
        ...
        def kernel(
            ...
            # Use i, j, k in the call to BGKNonNewtonian if needed
            if wp.static(isinstance(self.collision, BGKNonNewtonian)):
                _f_post_collision = self.collision.warp_functional(_f_post_stream, _feq, _rho, _u, i, j, k)
            else:
